...
export default function DeployBot() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      {/* ── GLOBAL CAPACITY BANNER — admin only ── */}
      {user?.is_admin && (
        serverFull ? (
          <div className="bg-red-500/10 border border-red-500/40 rounded-xl p-5 mb-5 text-center">
            ...
            <p className="text-gray-400 text-sm mb-3">
              ...
              Please try again another time — a slot may become available when another user removes their bot.
            </p>
      ...
      {/* Non-admin: only show the "server full" block if truly full */}
      {!user?.is_admin && serverFull && (
        <div className="bg-red-500/10 border border-red-500/40 rounded-xl p-5 mb-5 text-center">
          <div className="text-3xl mb-2">⛔</div>
          <h3 className="text-red-400 font-bold text-base mb-1">Service Unavailable</h3>
          <p className="text-gray-400 text-sm">
            All cloud server slots are currently full. Please try again another time.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-5">
          <h2 className="text-sm font-semibold text-white mb-3">🤖 Bot Name</h2>
          <input
            value={botName}
            onChange={e => setBotName(e.target.value.toLowerCase().replace(/[^a-z0-9-_]/g, ""))}
            ...
            {envKeys.map(key => (
              <div key={key}>
                ...
                <input
                  ...
                  disabled={serverFull}
                  placeholder={
                    key === "SESSION_ID" ? "Paste your SESSION_ID"
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 text-xs text-gray-400">
          ...
          <ol className="list-decimal list-inside space-y-1">
            ...
            <li>Copy the SESSION_ID and paste above</li>
          </ol>
        </div>
        ...
        {serverFull ? (
          ...
          <button
            ...
            disabled={!botName || missingRequired.length > 0 || !canDeploy || deployMut.isPending}
